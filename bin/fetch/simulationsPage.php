...
if ($simid != 0) {
	$ftchMus = "SELECT * FROM simulations WHERE active = 1 AND id = $simid LIMIT 1";
	...
	{
	echo "<div class='subrow' style='background:url(/graphics/simulations/".$id.".jpg) 30px 30px no-repeat #fff;' itemscope itemtype='http://schema.org/Organization'>";
	echo "<a href='$url_download' class='css3button downloadBtn' title='Download $name' target='_blank'>Download</a>";
	...
	if ($simid == 0) {
		$ftchAll = "SELECT id,name,description,fav,facebook FROM simulations WHERE active = 1 ORDER BY name ASC";
	} else {
		$ftchAll = "SELECT id,name,description,fav,facebook FROM simulations WHERE active = 1 AND museums_id = $musmid ORDER BY name ASC";
	}
	
	$result_all = mysql_query($ftchAll);
	while(list($id,$name,$description,$fav,$facebook)= mysql_fetch_row($result_all))
	{
	echo "<div class='subrow' style='background:url(/graphics/simulations/".$id.".jpg) 15px 15px no-repeat #fff;'>";

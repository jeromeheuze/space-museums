...
if (isset($_GET['id'])&&(is_numeric($_GET['id']))) {$musid = (int)$_GET['id'];} else {$musid = 0;}
if (isset($_GET['mid'])&&(is_numeric($_GET['mid']))) {$musmid = (int)$_GET['mid'];} else {$musmid = 0;}
if ($musid > 0) {
	$getFav = "SELECT fav FROM museums WHERE id='$musid'";
	$result_getFav = mysql_query($getFav);
	$num_rows_getFav = mysql_fetch_row($result_getFav);
	$currentFav = $num_rows_getFav[0] + 1;
	$queryFav = "UPDATE museums SET fav=$currentFav WHERE id='$musid'";
...
<head>
<meta charset="utf-8">
<title>SPACE Museums - Museums</title>
...
<meta itemprop="name" content="SPACE Museums - Museums">
...
<?php include("../includes/secondaryMenu.php"); ?>
    <div class="row">
    	<h1 class="htitles">Museums</h1>
        <div id="muselist" class="eMuse">
        <?php include("../bin/fetch/exhibits.php"); ?>
        </div>
        <?php include("../bin/fetch/museumsPage.php"); ?>
    </div>
    <div class="row mt10">
